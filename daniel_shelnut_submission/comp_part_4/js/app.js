...
$(document).ready(function(){
  ...
  $.getJSON(url,function(data){
    ...
    /*Shuffle function for data */
    let viewData=data.data;
    let shufffleData=[];
    while(viewData.length!==0){
      let randomIndex=Math.floor(Math.random()*viewData.length);
      shufffleData.push(viewData[randomIndex]);
      viewData.splice(randomIndex,1);
    }
    viewData=shufffleData
    let url="https://sv-reqres.now.sh/api/listings"
