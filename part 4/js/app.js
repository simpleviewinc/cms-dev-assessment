$(document).ready(() => {
  ...
  async function main() {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Get random number between 0 and the length of our data (not randomizing images because none of the image links from the API are working)
      let randomNumber = Math.floor(Math.random() * data.length);

      $(`.box-title${i}`).text(data[randomNumber].title);
    }
